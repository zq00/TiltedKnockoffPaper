...
# ============================================================
n_bins <- 5 # number of bins for Y (the scores are evaluated within each bin and then averaged across bins)
...
if (mode %in% c("both", "case")) {
  ...
  # calculate target correlation between X and Xtilde for each bins for CASES
  ...
  # parameters for the CASE machine 
  my_pars_case <- list(
    epochs        = 120L,
    epoch_length  = 20L,
    batch_size    = 10000L,
...
if (mode %in% c("both", "control")) {
  ...
  # calculate target correlation between X and Xtilde for each bins for CONTROLS 
  ...
  # parameters for CONTROL machine 
